...
class PluginManager:
        ...
        try:
            with self._lock:
                ...
                plugin_info = PluginInfo(
                    plugin=plugin_instance, metadata=metadata, status=PluginStatus.LOADED
                )
    ...
    def execute_pipeline(
        self, pipeline_index: int, initial_data: Dict[str, Any]
        ...
        with self._lock:
            return {
                **self.stats,
                ...
                "running_plugins": len(
                    [p for p in self.plugins.values() if p.status == PluginStatus.RUNNING]
                ),
                "error_plugins": len(
